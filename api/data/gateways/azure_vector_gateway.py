...
class AzureVectorGateway(VectorGateway):
    ...
    def filter(self, project_name: str, vector: npt.NDArray) -> list[VectorSearchResult]:
        headers = {"Content-Type": "application/json", "api-key": settings.VECTOR_DB_KEY }
        filter = AzureVectorFilter(project_name, vector)

        response = requests.post(settings.AZURE_URL, json=filter.to_dict(), headers=headers)
