...
class ConversationService:
    ...
    def __init__(
        self,
        llm: LLMGateway = Depends(OpenAILLMGateway),
        vector: VectorGateway = Depends(AzureVectorGateway),
    ):
        self.llm = llm
        self.vector = vector
    ...
    def completion(self, completion: CreateCompletion):
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            for _, response in zip(
                contents,
                executor.map(
                    lambda x: self._build_completion(completion.project_name, x),
                    contents,
                ),
            ):
                messages += response[0]
                sections_retrived += response[1]
                handover_to_human_needed = (
                    True if handover_to_human_needed else response[2]
                )
    ...
    def _build_completion(self, project_name: str, content: CompletionMessage):
        ...
        """
        It explicitly instructs the LLM not to respond with any additional information
        """
        system_message = "\n".join([i.content for i in contexts])
        system_message += "\n\nCRITICAL: Respond ONLY using the information and guidelines provided above. Do not add explanations, reasoning, or content beyond what is explicitly covered in these instructions. Be direct and concise."

        system_requirements = [
            LLMMessage(role=LLMMessageRole.SYSTEM, content=system_message),
            LLMMessage(
        ...
        messages = [
            content,
            CompletionMessage(role=CompletionRole.AGENT, content=system_response),
        ]
    ...
    def _need_human(self, contexts: list[VectorSearchResult]) -> bool:
        return any(
            context for context in contexts if context.type == VectorSearchResultType.N2
        )
