    ...
    def sample_completion_request(self):
        return CreateCompletion(
            ...
            messages=[
                ...
                CompletionMessage(role=CompletionRole.AGENT, content="What car models does Tesla offer?"),
                CompletionMessage(role=CompletionRole.USER, content="HCan I charge a Tesla at home?")
            ]
        )

    def test_completion_success_with_user_messages(self, conversation_service, sample_completion_request):
        ...
        user_messages = [msg for msg in result.messages if msg.role == CompletionRole.USER]
        assert len(user_messages) == 2
        
        agent_messages = [msg for msg in result.messages if msg.role == CompletionRole.AGENT]
        assert len(agent_messages) == 2

    def test_completion_handover_to_human_needed(self, conversation_service, mock_vector_gateway):
